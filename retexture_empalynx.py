"""Re-bake texture on 2-empalynx using front + back + side images."""
...
NAME = "2-empalynx"
INPUT  = ROOT / "input" / "2-empalynx"
OUTPUT_3D  = ROOT / "output" / NAME / "3dmodel"
OUTPUT_TEX = ROOT / "output" / NAME / "textures"

# Use the unrigged textured GLB as source — same geometry as Hunyuan3D output
...
image_paths = {
    "front": INPUT / "front_processed.webp",
    "back":  INPUT / "back_processed.webp",
    "side":  INPUT / "side_processed.webp",
}
...
print(f"  {rig_msg}")
if rigged:
    dest = OUTPUT_3D / f"{NAME}_textured_rigged.glb"
    shutil.copy2(rigged, dest)
    print(f"Saved textured rigged GLB: {dest}")
    if rigged_fbx:
        fbx_dest = OUTPUT_3D / f"{NAME}_textured_rigged.fbx"
        shutil.copy2(rigged_fbx, fbx_dest)
        print(f"Saved textured rigged FBX: {fbx_dest}")
else:
